...
# ==== 放到 eval.py 顶部（或 torch.load 调用之前） ====
import torch, numpy as np
from torch.serialization import add_safe_globals

def _allowlist_numpy_for_torch_load():
    """
    为 PyTorch 2.6+ 的 weights_only=True 放行常见 NumPy 类型（适配 NumPy 1.x / 2.x）。
    只有在你信任 checkpoint 来源时才这样做。
    """
    # 基础类
    base_allow = [
        np.dtype,                 # numpy.dtype 本体
        np.generic,               # 所有 numpy 标量的基类
        np.number, np.floating, np.integer,
        np.bool_, np.void,
        np.core.multiarray.scalar # 常见的 numpy scalar 类型（老版本提示）
    ]
    add_safe_globals(base_allow)

    # 1) 放行所有常见 dtype“类”（注意：NumPy 2.x 的 dtype 类在 numpy.dtypes.*）
    #    例如 numpy.dtypes.Float64DType、Int64DType 等
    common_dtype_names = [
    ...
    dtype_classes = {np.dtype(n).__class__ for n in common_dtype_names}
    add_safe_globals(list(dtype_classes))

    # 2) 放行所有常见标量类型（np.float64/np.int64 等）
    scalar_types = [
        np.float64, np.float32, np.float16,
        np.int64, np.int32, np.int16, np.int8, np.uint8,
        np.bool_, np.complex64, np.complex128
    ]
    add_safe_globals(scalar_types)



def _tensor_to(t, device):
    if t is None: 
        return None
    return t.to(device=device, dtype=torch.float32, non_blocking=True)

def _pc_to_device(pc, device):
    # 常见 nn.Parameter / buffer
    for name in [
        "_xyz", "_scaling", "_rotation", "_opacity",
        "_features_dc", "_features_rest", "max_radii2D"
    ]:
        if hasattr(pc, name) and getattr(pc, name) is not None:
            p = getattr(pc, name)
            # nn.Parameter 的 .data 迁移可避免替换对象引用
            if hasattr(p, "data"):
                p.data = _tensor_to(p.data, device)
            else:
                setattr(pc, name, _tensor_to(p, device))

    # 如果 GaussianModel / Scene 里还有按帧缓存（示例字段名，按你的实现增减）
    for name in ["world_xyz_cache", "cov3D_cache"]:
        if hasattr(pc, name) and getattr(pc, name) is not None:
            setattr(pc, name, _tensor_to(getattr(pc, name), device))

def _sensors_to_device(scene, device):
    # 你的 LiDARSensor / Camera 里常见的标定量（按实际字段增减）
    if hasattr(scene, "lidar"):
        lidar = scene.lidar
        for name in [
            "sensor_center",          # [T,3]
            "sensor2world",           # [T,4,4] 或 [T,3,4]
            "inclination_bounds",     # LiDAR 垂直角
            "angle_offset", "pixel_offset"
        ]:
            if hasattr(lidar, name):
                v = getattr(lidar, name)
                if isinstance(v, torch.Tensor):
                    setattr(lidar, name, _tensor_to(v, device))
    if hasattr(scene, "cameras"):
        for cam in scene.cameras:
            for name in ["world_view_transform", "projection_matrix", "camera_center"]:
                if hasattr(cam, name):
                    v = getattr(cam, name)
                    if isinstance(v, torch.Tensor):
                        setattr(cam, name, _tensor_to(v, device))
...
class LiDARRTMeter:
    def __init__(self, args) -> None:
        ...
        # 调用一次即可
        _allowlist_numpy_for_torch_load()
        model_params, first_iter = torch.load(self.args.model_path, weights_only=True, map_location="cpu")
        ...
        for pc in gaussians:
            _pc_to_device(pc, device)
        _sensors_to_device(scene, device)
    ...
    def record_render(self, gaussians, background, scene, frame_id, args):
        ...
        points, _ = scene.train_lidar.inverse_projection(frame_id) #!读取这一帧的点云
        ...
        if self.unet:
            ...
            print("self.unet.training: ", self.unet.training)
        ...
        #! 从世界坐标系下转换到传感器坐标系下
        sensor2world = scene.train_lidar.sensor2world[frame_id]
...
def parse_args():
    ...
    parser.add_argument("-s", "--save", type=str, default="./val/kitti_dropoutmask", help="save path")
    parser.add_argument(
