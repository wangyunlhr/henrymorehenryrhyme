...
class SceneLidar(Scene):
    def __init__(self, args, waymo_raw_pkg, shuffle=True, resize_ratio=1, test=False):
        ...
        all_intensity = []
        all_normals = []
        for frame in range(frame_range[0], frame_range[1] + 1): #!无论训练帧还是测试帧都加载
            # if frame in eval_frames: # 只在训练阶段初始化背景点云时跳过测试帧
            #     print('Skipping frame {} for background point initialization'.format(frame))
            #     continue #没有影响
            lidar_pts, lidar_intensity = lidar.inverse_projection(frame) #!世界坐标系下
