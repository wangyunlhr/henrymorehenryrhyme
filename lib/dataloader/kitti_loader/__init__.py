...
dynamic_label_names = {
    'person', 'rider',
    'car', 'truck', 'bus', 'caravan', 'trailer',
    'train', 'motorcycle', 'bicycle',
    'unknown vehicle',
}

def load_lidar_bbox(lidar_bbox_dir, full_seq, frame_length, using_cache=True):
    ...
    for obj in root:
        ...
        labels.add(object_type)
        if (
            metadata["timestamp"] < frame_length[0]
            or metadata["timestamp"] > frame_length[1]
        ):
            continue
        # if object_type not in ["car", "truck", "bus"]:
        #     continue
        if object_type not in dynamic_label_names: #!换成了更多的类别检查
            continue
    ...
    print(f"Loaded {len(bboxes)} bounding boxes with labels: {labels}")
    return bboxes
...
#!
def load_lidar_bbox_all(lidar_bbox_dir, full_seq, args, using_cache=True):
    bboxes: dict[str, BoundingBox] = {}
    bbox_pickle_dir = os.path.join(lidar_bbox_dir, "cache")
    os.makedirs(bbox_pickle_dir, exist_ok=True)
    bbox_pickle_path = os.path.join(bbox_pickle_dir, f"{full_seq}.pkl")
...
def load_kitti_raw(base_dir, args):
    ...
    for frame in range(frames[0], frames[1] + 1):
        ...
        lidar.add_frame(frame, last_ego2world, range_image_r1, range_image_r2, direct_point=xyzs, direct_intensity=intensities) #!kitti添加直接点云
...
#! 加载kitti全部帧的数据
def load_kitti_raw_all(base_dir, frame_length, seq):
    ...
    print('ego2world length:', len(ego2world))
    W, H = 1030, 66
    inc_buttom, inc_top = math.radians(-24.9), math.radians(2.0)
    azimuth_left, azimuth_right = np.pi, -np.pi
    max_depth = 80.0
    ...
    if frames[0] not in ego2world.keys():
        for pre_frame in range(frames[0] - 1, -1, -1):
            if pre_frame in ego2world.keys():
                last_ego2world = ego2world[pre_frame]
                break
    if last_ego2world is None:
        last_ego2world = np.eye(4, dtype=np.float32)
        print('last_ego2world init as identity', '='*10)

    for frame in range(frames[0], frames[1] + 1):
        # print('load_kitti_raw_all frame:', frame)
        xyzs, intensities = lidar_points[frame][:, :3], lidar_points[frame][:, 3]
        x, y, z = xyzs[:, 0], xyzs[:, 1], xyzs[:, 2]
        dists = np.linalg.norm(lidar_points[frame][:, :3], axis=1)
        ...
        if frame in ego2world.keys():
            last_ego2world = ego2world[frame]
            print(frame, ' use its own ego2world')
        range_image_r1[range_image_r1 == -1] = 0
        range_image_r2[range_image_r2 == -1] = 0
        # lidar.add_frame(frame, last_ego2world, range_image_r1, range_image_r2)
    ...
    return lidar, lidar_bbox, frames