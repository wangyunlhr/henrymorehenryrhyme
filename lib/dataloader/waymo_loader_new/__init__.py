...
def load_waymo_raw(fp):
    ...
    data_type = "Waymo" #!固定是waymo数据
    # pbar = tqdm(total=(args.frame_length[1] + 1 - args.frame_length[0]))
    # pbar = tqdm(desc="Loading Waymo frames")
    # for frame in range(args.frame_length[0], args.frame_length[1] + 1):

    for frame_idx, record in enumerate(tqdm(tf.data.TFRecordDataset(fp, compression_type=""),
                                        desc=f"Loading Waymo frames: {os.path.basename(fp)}")): #! 遍历所有帧
        # record = dataset[frame]

        frame_data = dataset_pb2.Frame()
        frame_data.ParseFromString(bytearray(record.numpy()))
        scene_id = frame_data.context.name

        for i in range(5):

            name = frame_data.context.laser_calibrations[i].name #! 只取top lidar
            if name != 1:
                continue
            ...
            #     decompressed_dir, f"decompressed_frame_{frame_idx}_sensor_{name}.pt"
            # )
            ...
            for lidar_data in frame_data.lasers:
                ...
                if lidar_data.name == name:
                    ...
                    range_image_r1[..., 0:2][range_image_r1[..., 0:2] == -1] = 0 #!无效值为0  #!只处理了rangeimage1
                    range_image_r2[..., 1] = torch.clamp(
                        range_image_r2[..., 1], max=1
                    ) #! intensity最大值1
                    range_image_r2[..., 0:2][range_image_r2[..., 0:2] == -1] = 0 #!无效值为0  #!只处理了rangeimage1

                # torch.save((range_image_r1, range_image_r2), decompressed_path)

            lidar.add_frame(
                frame=frame_idx, ego2world=ego2world, r1=range_image_r1, r2=range_image_r2
            )
        ...
        for labels in frame_data.laser_labels:
            ...
            if id not in bboxes:
                ...
                size = torch.tensor(float_data).float().cuda() #!nocuda
                bboxes[id] = BoundingBox(object_type, object_id, size)
            bboxes[id].add_frame_waymo(frame_idx, metadata, ego2world) #! nocuda

    #     pbar.update(1)
    # pbar.close()
    return lidar, bboxes, scene_id
