...
class GaussianModel:
    ...
    def __init__(self, dimension, sh_degree : int, extent : int = 200, bounding_box : BoundingBox= None, dynamic_flag = False):
    ...
    def get_world_xyz_and_normals(self, timestamp = 0.0):
        if self.bounding_box is not None and timestamp in self.bounding_box.frame:
            R = build_rotation(self.bounding_box.frame[timestamp][1]).squeeze(0)
            device = R.device
            if self._xyz.device != device:
                self._xyz = self._xyz.to(device)
            if self._normals.device != device:
                self._normals= self._normals.to(device)
            return self._xyz @ R.T + self.bounding_box.frame[timestamp][0], self._normals @ R.T
        else:
            return self._xyz, self._normals


    @property
    def get_local_xyz(self): #!注意，因为xyz_set重置了，所以不再是local坐标系了
        return self._xyz
    ...
    def create_from_pcd(self, pcd, use_normals=False):
        ...
        color_intensity = pcd.color_intensity.float().cuda() # N x 3
        fused_color = RGB2SH(color_intensity) # N x 3
        ...
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, self.dimension) # N x 2 
    ...
    def xyz_set(self, new_xyz, new_intensity, new_normals, new_labels):
        self._xyz = new_xyz
