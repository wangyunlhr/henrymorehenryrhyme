...
class LiDARSensor:
    def __init__(self, sensor2ego, name, inclination_bounds, data_type):
        ...
        self.direct_point = {} #! kitti直接添加点云
        self.direct_intensity = {}
        self.pixel_pose = {}
        self.H, self.W = 0, 0
        self.num_frames = 0
        self.train_frames = []
    ...
    def add_frame(self, frame, ego2world, r1, r2, pixel_pose=None, direct_point = None, direct_intensity = None): #!直接添加点云
        # print('inside_add_frame0')
        ...
        self.num_frames += 1
        if direct_point is not None: #!kitti直接添加点云
            if isinstance(direct_point, np.ndarray):
                direct_point = torch.from_numpy(direct_point)
            self.direct_point[frame] = direct_point.float().cpu()
        if direct_intensity is not None:
            if isinstance(direct_intensity, np.ndarray):
                direct_intensity = torch.from_numpy(direct_intensity)
            self.direct_intensity[frame] = direct_intensity.float().cpu()
        # print('inside_add_frame2')
